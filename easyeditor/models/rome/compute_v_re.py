...
from .const_subob import re_sub_obj
...
def compute_v(
    ...
    sub_ob_replace=re_sub_obj[request["relation_id"]]
    
    all_subjects=[i[0] for i in sub_ob_replace]
    all_objects=[i[1] for i in sub_ob_replace]
    
    print("subjects and objects \n:", all_subjects,"\n",all_objects)
    ...
    #*****************replace**********************
    input_re_sub_hrt = tok(
        all_subjects,
        return_tensors="pt",
        padding=True,
    ).to(f"cuda:{hparams.device}")
    
    input_re_obj_hrt = tok(
        all_objects,
        return_tensors="pt",
        padding=True,
    ).to(f"cuda:{hparams.device}")
    ...
    hidden_re_inv_sub = model(**input_re_sub_hrt, output_hidden_states=True).hidden_states[model_layers].squeeze(0).mean(dim=1).mean(dim=0)
    hidden_re_inv_obj = model(**input_re_obj_hrt, output_hidden_states=True).hidden_states[model_layers].squeeze(0).mean(dim=1).mean(dim=0)
    print(hidden_re_inv_sub.shape)
    hidden_re_inv_replace=hidden_re_inv_sub-hidden_re_inv_obj
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        #mean 模式
            ...
            hrt_inv = hidden_obj[i][o[0]:o[1]].mean(dim=0)+hidden_re_inv_replace-hidden_hrt[i][s[0]:s[1]].mean(dim=0)
            ...
            hrt_true_inv=hidden_obj_true[i][o_t[0]:o_t[1]].mean(dim=0) + hidden_re_inv_replace - hidden_hrt[i][s[0]:s[1]].mean(dim=0)
