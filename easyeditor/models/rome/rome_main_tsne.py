...
from .compute_return import compute_v
...
def apply_rome_to_model(
    ...
    **kwargs
    ...
    for i, request in enumerate(requests):
        ...
        deltas,key,value = execute_rome(model, tok, request, hparams)
    ...
    return model, weights_copy, upd_matrixs, [key.detach().clone().cpu().numpy(),value.detach().clone().cpu().numpy()]
...
def execute_rome(
    ...
    for layer in sorted(hparams.layers):
        ...
        right_vector,key,value = compute_v(
            model,
            tok,
            request,
            hparams,
